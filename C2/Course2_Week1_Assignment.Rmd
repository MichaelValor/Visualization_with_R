...
dat <- cel %>% filter(congress == 115) %>% rename(`Gender`=`female`)
dat$Gender <- dat$Gender %>% recode(`0`="male", `1`="female")

dat %>% ggplot(aes(x=dwnom1, y=all_pass, color=Gender)) + geom_point() + labs(x="Ideology", y="Bills Passed")
```
...
dat$majority <- dat$majority %>% recode(`0`="minority", `1`="majority")

dat %>% ggplot(aes(x=votepct, y=all_pass, color=Gender)) + geom_point() + labs(x="Vote Percentage", y="Bills Passed") + scale_color_manual(values = c("orange", "green")) + facet_wrap(~majority)
```
...
dat %>% ggplot(aes(x=majority, y=les)) + geom_boxplot() + labs(x="Majority or Minority", y="Legislative Effectiveness", title="LES in the 115th Congress")
```
