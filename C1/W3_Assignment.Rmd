...
sum(vec1)
```
The sum of numbers of the vector is `r sum(vec1)`.
...
```{r,problem2}
my_dat = data.frame("alpha" = c(1:5), "beta" = c(6:10))
my_dat
```
...
```{r,problem4}
boxplot(dat$faminc_new, xlab="Family Income", main="Sijia Gao´s Boxplot of Family Income")
```
...
# str(filter(dat, gender==1 & region %in% c(1, 2) & marstat == 1 & newsint==1))
filtered_dat = dat %>% filter(gender==1 & (region == 1 | region == 2) & marstat==1 & newsint==1)
str(filtered_dat)
```
...
survey1 <- recode(dat$CC18_325d, `1`=1, `2`=0)
survey2 <- recode(dat$CC18_325a, `1`=1, `2`=0)

dat_new <- dat %>% mutate(tax_scale = survey1 + survey2)
head(dat_new$tax_scale,20)

dat7 <- dat %>% mutate(tax_scale=(CC18_325d==1)+(CC18_325a==1)) 
```
...
```{r problem9}
by_region <- dat %>% group_by(region)
summarise(by_region, Trump_Approve_Mean = mean(CC18_308a, na.rm = TRUE))
...
```{r problem10}
p10_dat <- dat %>% filter(investor == 2 & faminc_new >= 5 & faminc_new <= 10)
summarise(p10_dat, 
          "Mean Religion Imp." = mean(pew_religimp),
          "Median Religion Imp." = median(pew_religimp),
          "Standard Dev. Religion Imp." = sd(pew_religimp))
dat %>% filter(investor==2 & faminc_new %in% c(5:10)) %>% summarise("Mean Religion Imp."=mean(pew_religimp), "Median Religion Imp."=median(pew_religimp), "Standard Dev. Religion Imp."=sd(pew_religimp))
```
...
qplot(pid7, data = p12, geom = "histogram", xlab="Seven Point Party ID", ylab="Count")
